...
import { MdInvertColors, MdBrightnessLow, MdFormatColorReset, MdLinearScale, MdPhotoSizeSelectSmall } from 'react-icons/md';
...
const VariationsControls = ({ baseColor }) => {
  ...
  useEffect(() => {
    ...
    document.body.style.baseColor = foregroundColor;

    const hslHarmonies = getBaseAndHarmonies(baseColor, harmonyQuantity);
    const hslInverses = getInverses(baseColor, hslHarmonies, inverseQuantity);
    const hslAnalogousColors = getAnalogousColors(baseColor, analogousQuantity);
    baseHarmoniesInverseAndAnalogousColorList = getBaseHarmoniesInverseAndAnalogousColorList(baseColor, hslHarmonies, hslInverses, hslAnalogousColors);
    ...
    const cssClasses = (getCssClasses(baseColor, exportHexToggled, exportHslToggled, exportRgbToggled)) + (getCssClasses(hslHarmonies, exportHexToggled, exportHslToggled, exportRgbToggled)) + (getCssClasses(hslInverses, exportHexToggled, exportHslToggled, exportRgbToggled)) + (getCssClasses(hslLighters, exportHexToggled, exportHslToggled, exportRgbToggled)) + (getCssClasses(hslDarkers, exportHexToggled, exportHslToggled, exportRgbToggled)) + (getCssClasses(hslDarkers, exportHexToggled, exportHslToggled, exportRgbToggled));
  ...
  let hslHarmonies = getBaseAndHarmonies(baseColor, harmonyQuantity);
  let hslInverses = getInverses(baseColor, hslHarmonies, inverseQuantity);
  let hslAnalogousColors = getAnalogousColors(baseColor, analogousQuantity);
  let baseHarmoniesInverseAndAnalogousColorList = getBaseHarmoniesInverseAndAnalogousColorList(baseColor, hslHarmonies, hslInverses, hslAnalogousColors);
  ...
  const makeColorSwatches = (colorSet, swatchSize) => {
    if(colorSet.length) {
      return colorSet.map((singleColor, i) => {
        const key = (singleColor.matchRelationship + (Number(i) + 1));
        const hslString = hslToString(singleColor);
        const rgb = hslToRgb(singleColor.h, singleColor.s, singleColor.l);
        const rgbString = rgbToString(rgb);
        const hexString = hslToHex(singleColor.h, singleColor.s, singleColor.l);
        return (
          <div key={key} style={{ background: `${hslString}`, height: `${swatchSize}rem`, width: `${swatchSize}rem` }} className={styles.Swatch} onClick={handleSwatchClick}>
            <aside className={`${styles.details} ${swatchToggled && styles.hidden}`}>
              <strong>{(singleColor.matchRelationship)}</strong>
              {exportHexToggled &&
  ...
  return (
    <>
      <div className={styles.VariationsControls} style={{ background: `hsl(${(baseColor.h)}, ${baseColor.s * 100}%, ${baseColor.l * 150}%)` }} >
      ...
      <section className={styles.colorFormats} style={{ borderColor: `hsl(${(getOppositeDegree(baseColor.h))}, ${baseColor.s * 100}%, ${baseColor.l * 25}%)`, borderWidth: '0 2px 2px', background: `hsl(${(getOppositeDegree(baseColor.h))}, ${baseColor.s * 100}%, ${baseColor.l * 50}%)`, borderStyle: 'solid' }}>
